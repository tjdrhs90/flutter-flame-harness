...
- **결과물 게임 위치:** `/Users/ssg/AndroidStudioProjects/<game-slug>/` (기존 7개 게임과 나란히).

## 3. 전역 제약 (Global Constraints)

- **플러그인 이름:** `flutter-flame-harness` · 스킬 접두사 `flame-harness-*`.
- **플러그인 소스 위치:** `/Users/ssg/AndroidStudioProjects/flutter-flame-harness/` (자체 git 저장소).
...
- **중앙 인증키 금고:** `/Users/ssg/AndroidStudioProjects/credentials/` — `AuthKey_339MZ7CUZ5.p8`, `play-store-key.json`, `upload-keystore.jks`, `store-metadata.md`.
...
- **iOS 자격:** Issuer `f9a69502-1e93-4fd1-9f53-5eb4db1b637a`, Key `339MZ7CUZ5`, Team `8DHJJJ66LY`.
- **개발자 정보(store-metadata.md):** Seonggon Sim / tjdrhs90@gmail.com / privacy `https://tjdrhs90.github.io/privacy/` / 지원 `https://tjdrhs90.github.io` / `Copyright 2026. Gonigon all rights reserved.`
- **번들 ID:** `com.gonigon.<slug>` (plan 단계 확정, iOS/Android 동일).
- **Android 서명:** 공유 `upload-keystore.jks` (alias `upload`, store/key pw `111111`) 전 게임 공유. 누락 시에만 keytool로 신규 생성.
...
- `lib/` 구조 매핑(`game/`, `game/components/`, `game/systems/`, `game/data/`, `screens/`, `ui/`, `l10n/`), 앱이름·slug·번들ID `com.gonigon.<slug>` 확정 → `config.md`.
