...
플러그인은 설치 시점에 캐시로 복사되므로, 소스를 고쳐도 다시 동기화해야 반영됩니다. 새 커밋을 받은 뒤
(릴리스마다 버전이 올라감):

```bash
/plugin marketplace update flutter-flame-harness   # 최신 버전을 캐시로 다시 가져옴
/reload-plugins                                     # 현재 세션에 적용
```

`/plugin` 메뉴에서 새 버전을 확인하세요. 변경이 안 보이면 재설치:
...
(캐시가 꼬이면 최후의 수단: `rm -rf ~/.claude/plugins/cache` 후 Claude Code 재시작.)
